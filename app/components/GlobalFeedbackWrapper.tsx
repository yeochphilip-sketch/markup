...
export default function GlobalFeedbackWrapper({ children }: { children: React.ReactNode }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      console.error('Platform feedback routing error:', err);
    }
  };
  ...
  return (
    <>
      ...
      {/* Platform-wide floating feedback trigger icon button */}
      <button 
        onClick={() => setIsOpen(true)}
        className="fixed bottom-6 right-6 w-12 h-12 bg-slate-900 hover:bg-slate-800 border border-slate-800 text-indigo-400 rounded-full flex items-center justify-center shadow-2xl transition-all duration-200 hover:scale-105 group z-50 animate-bounce"
        title="Flag issue or leave feedback"
      >
